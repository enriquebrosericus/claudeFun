#!/usr/bin/env python3
"""
Backfill 2026 spring training player stats (game-by-game) into PostgreSQL.

Fetches each player's spring training game log (gameType=S) from the MLB
Stats API, accumulates counting stats game-by-game, and inserts one row per
game date into player_batting / player_pitching with game_type='S'.

Re-run any time to refresh (ON CONFLICT DO UPDATE).
...
session.headers.update({"User-Agent": "MLBSpringBackfill/2.0", "Accept": "application/json"})
...
def backfill_batter(cur, person_id: int, name: str, pos: str) -> int:
    data = api_get(f"/people/{person_id}/stats",
                   stats="gameLog", season=SEASON, gameType=GAME_TYPE,
    ...
    for game in splits:
        st   = game.get("stat", {})
        date = datetime.date.fromisoformat(game.get("date", f"{SEASON}-01-01"))
        ...
        cur.execute("""
            ...
            ON CONFLICT (date, player_id, season, game_type) DO UPDATE SET
                ...
                iso=EXCLUDED.iso
        """, (date, SEASON, name, person_id, TEAM_ABBR, GAME_TYPE, pos,
